...
/**
 * This class will be used to make a http call to the new PIM grouping integration service. This class will
 * also use the XSL stylesheet to parse the input request to the specified format for the grouping integration. 
 * 
 */
public class BelkGetGroupIntegrationResponse {
	
	private final GroupIntegrationRequestData requestData;
    ...
    private static final Log log = LogFactory.getLog(BelkGetGroupIntegrationResponse.class);
	
	static {
		createJaxbContext();
	}
	
	public BelkGetGroupIntegrationResponse(GroupIntegrationRequestData requestData) {
	...
	private static Transformer createTransformerInstance() {
		if (inputTransformer == null) {
			...
			try {
				inputTransformer = factory.newTransformer(getXSLTemplate("/Group_Integration_Request.xsl"));
	...
	/**
	 * Creates a singleton JAXBContext for the GroupIntegrationRequestData class.
	 * @return - JAXBContext
	 */
	private static JAXBContext createJaxbContext() {
		if (createContext ==  null) {
			try {
				createContext = JAXBContext.newInstance(GroupIntegrationRequestData.class);
	...
	private static Source getXSLTemplate(String fileName) {
		if (xslSource == null) {
			InputStream xslInputStream = BelkGetGroupIntegrationResponse.class.getClassLoader().getResourceAsStream(fileName);
	...
	private String sendRequestToPIM(HttpURLConnection httpCon, String xmlContents) throws SocketTimeoutException {
		...
		if (log.isInfoEnabled()) {
			log.info("Sending request to PIM webservice");
		}
		
		try {
             if (log.isInfoEnabled()) {
                 log.info("Entered Try Block in sendRequestToPIM");
             }
		...
		if (log.isDebugEnabled()) {
			log.debug("Response Xml " + responseContents.toString());
		}
    ...
    private String getServiceURL() throws BelkProductIntegrationException {
        ...
        if (log.isDebugEnabled()) {
        	log.debug("PIM Web Service URL is " + properties.get("PIMCarIntegrationServiceWSDL"));
        }
