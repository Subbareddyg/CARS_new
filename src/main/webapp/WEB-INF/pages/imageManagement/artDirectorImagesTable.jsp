...
<c:set var="imageStatusTitle">
	Image Status<a class="imageTypeHelpIcon" href="javascript:showArtDirectorImageStatusHelp();"></a>
</c:set>
</c:if>
<c:if test="${sessionScope.isPreviewPage == 'true'}">
<c:set var="imageStatusTitle">
...
<c:if test="${sessionScope.isPreviewPage != 'true'}">
  <display:column style="width:2%;" sortable="false">
   	<c:if test="${(carImage.image.imageTrackingStatus.imageTrackingStatusCd=='REQUESTED' && carImage.image.imageSourceType.imageSourceTypeCd == 'VENDOR') 
  						|| (carImage.image.imageSourceType.imageSourceTypeCd == 'ON_HAND' && carImage.image.imageTrackingStatus.imageTrackingStatusCd=='APPROVED')}">
